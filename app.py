...
<p style='
    ...
    font-size:16px;
'>
당첨 횟수 데이터를 참고하거나 자동 방식으로 행운 번호를 생성할 수 있습니다.
</p>
...
""", unsafe_allow_html=True)
    ...
    if mode == "당첨 횟수 데이터 기반 생성":

        st.caption("📊 과거 당첨 번호 출현 데이터를 참고해 번호를 생성합니다.")

    else:

        st.caption("🎲 완전 랜덤 방식으로 번호를 생성합니다.")
...
""", unsafe_allow_html=True)
        ...
        if user_input:
            ...
            try:
                ...
                # 숫자 범위 검사
                valid = all(1 <= n <= 45 for n in numbers)

                if not valid:

                    st.warning("1~45 사이 숫자만 입력 가능합니다.")

                elif len(numbers) > 5:

                    st.warning("고정 번호는 최대 5개까지 입력 가능합니다.")

                else:

                    user_numbers = sorted(numbers)

                    st.caption(f"선택된 번호 : {user_numbers}")
...
if st.button("행운 번호 받기"):
    ...
    # 고정 번호 검사
    elif len(user_numbers) > 5:

        st.error("고정 번호는 최대 5개까지 선택 가능합니다.")
